...
# ==========================================
@st.cache_data(ttl=300) # Refreshes every 5 minutes automatically
def load_data():
    df = pd.DataFrame()
    is_live = False
    ...
    if JIRA_EMAIL and JIRA_TOKEN:
        try:
            ...
            # Fetch latest tickets from your SVF project
            issues = jira.search_issues('project = SVF ORDER BY created DESC', maxResults=1000)
            ...
            pass # We will handle the fallback silently
            
    # 2. Fallback to your offline CSV if Live fails or secrets aren't set
...
if is_live:
    st.sidebar.success("🟢 Live Connection Active")
else:
...
with tab_overview:
    ...
    with g_col1:
        # Gauge 1: Resolution SLA 
        fig_res_sla = go.Figure(go.Indicator(
            mode = "gauge+number",
            value = 96.2,
            title = {'text': "Resolution SLA Met %"},
    ...
    with g_col2:
        # Gauge 2: First Response SLA 
        fig_fr_sla = go.Figure(go.Indicator(
            mode = "gauge+number",
            value = 94.3,
            title = {'text': "First Response SLA Met %"},
    ...
    with g_col3:
        # Gauge 3: Satisfaction
        fig_sat = go.Figure(go.Indicator(
            mode = "gauge+number",
            value = avg_sat,
            title = {'text': "Average CSAT"},
    ...
    with g_col4:
        # Chart 4: Satisfaction Breakdown
        if 'Satisfaction rating' in df.columns:
...
with tab_sat:
    ...
    if 'Satisfaction rating' in df.columns:
        ...
        st.warning("No Satisfaction data available in the current dataset.")
