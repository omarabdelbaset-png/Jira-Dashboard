...
# --- SAFE CHART FUNCTIONS ---
def s_bar(d, m, t):
    if d.empty: return pc(go.Figure().update_layout(title=t))
...
def s_brc(d, m, gc, t, c):
    if d.empty or d[d[m]=="Breached"].empty: return pc(go.Figure().update_layout(title=t))
...
def safe_pie(d, m, t):
    if d.empty: return pc(go.Figure().update_layout(title=t))
    vc = d[m].value_counts().reset_index(name="C")
    if vc.empty: return pc(go.Figure().update_layout(title=t))
    fig = px.pie(vc, names=m, values="C", hole=.5, color=m, color_discrete_map={"Met":"#00CC96","Breached":"#EF553B"}, title=t)
    if len(vc) > 1: fig.update_traces(pull=[0.05,0])
    pc(fig)
...
with t3:
    ...
    c5.metric("Avg Act FR", f"{df.get('Act_TFR', pd.Series(dtype=float)).clip(lower=0).median():.1f} hrs" if "Act_TFR" in df and not df["Act_TFR"].isna().all() else "N/A")
    c6.metric("Avg Act Res", f"{df.get('Act_Res', pd.Series(dtype=float)).clip(lower=0).median():.1f} hrs" if "Act_Res" in df and not df["Act_Res"].isna().all() else "N/A")
    
    st.divider()
    c1, c2 = st.columns(2)
    with c1: safe_pie(tfr, "TFR_met", "FR SLA Met vs Breached")
    with c2: safe_pie(ttr, "TTR_met", "Res SLA Met vs Breached")
...
with t4:
    ...
    else: st.info("No satisfaction data available yet.")
