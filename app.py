...
t1, t2, t3, t4, t5 = st.tabs(["📈 Overview", "🎫 Ticket Analysis", "🚦 SLA Performance", "⭐ Satisfaction", "📅 Trends & Raw Data"])
...
with t1:
    ...
    s1.metric("Resolution SLA Met", f"{ttr_p:.1f}%", f"-{int((ttr['TTR_met']=='Breached').sum())} breached", "inverse")
    s2.metric("First Response SLA Met", f"{tfr_p:.1f}%", f"-{int((tfr['TFR_met']=='Breached').sum())} breached", "inverse")
    s3.metric("Avg Satisfaction", f"{s_avg:.2f}/5", f"{len(sat)} ratings")
    s4.metric("5-Star Ratings", f"{int((sat['Satisfaction']==5).sum())}", f"{100*int((sat['Satisfaction']==5).sum())/len(sat):.1f}%" if len(sat) else "0%")

    def gauge(v, t, mx=100, c="#00CC96"):
        return go.Figure(go.Indicator(mode="gauge+number+delta" if mx==100 else "gauge+number", value=v, title={"text":t}, number={"suffix":"/5" if mx==5 else "%"}, delta={"reference":80} if mx==100 else None, gauge={"axis":{"range":[0 if mx==100 else 1, mx]},"bar":{"color":c},"threshold":{"line":{"color":"orange","width":3},"value":80 if mx==100 else 4}}))
